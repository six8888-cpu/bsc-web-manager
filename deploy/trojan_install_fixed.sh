...
# 使用ghproxy镜像加速
download_url="https://ghproxy.com/https://github.com/Jrohy/trojan/releases/download/"
version_check="https://ghproxy.com/https://api.github.com/repos/Jrohy/trojan/releases/latest"
service_url="https://ghproxy.com/https://raw.githubusercontent.com/Jrohy/trojan/master/asset/trojan-web.service"
...
installDependent(){
    ...
    if [[ ${package_manager} == 'dnf' || ${package_manager} == 'yum' ]];then
        ${package_manager} install socat crontabs bash-completion curl wget -y
    else
        ${package_manager} update
        ${package_manager} install socat cron bash-completion xz-utils curl wget -y
    fi
}
...
installDocker() {
    if command -v docker &> /dev/null; then
        colorEcho $green "Docker已安装"
        return 0
    fi
    
    colorEcho $blue "正在安装Docker（使用阿里云镜像）..."
...
EOF
    ...
    if docker --version &> /dev/null; then
        colorEcho $green "Docker安装成功"
    else
        colorEcho $red "Docker安装失败"
        exit 1
    fi
}
...
installTrojan(){
    ...
    colorEcho $blue "正在获取最新版本信息..."
    # 使用多种方式获取版本信息
    lastest_version=$(curl -H 'Cache-Control: no-cache' -s --connect-timeout 10 "$version_check" | grep 'tag_name' | cut -d\" -f4)
    
    # 如果ghproxy失败，尝试直接访问GitHub API
    if [[ -z $lastest_version ]]; then
        colorEcho $yellow "使用备用方式获取版本..."
        lastest_version=$(curl -H 'Cache-Control: no-cache' -s --connect-timeout 10 "https://api.github.com/repos/Jrohy/trojan/releases/latest" | grep 'tag_name' | cut -d\" -f4)
    fi
    
    # 如果还是失败，使用固定版本
    if [[ -z $lastest_version ]]; then
        lastest_version="v2.10.6"
        colorEcho $yellow "无法获取最新版本，使用版本: $lastest_version"
    fi
    ...
    # 尝试使用ghproxy下载
    if ! curl -L --connect-timeout 30 "$download_url/$lastest_version/$bin" -o /usr/local/bin/trojan; then
        colorEcho $yellow "ghproxy下载失败，尝试直接从GitHub下载..."
        curl -L --connect-timeout 30 "https://github.com/Jrohy/trojan/releases/download/$lastest_version/$bin" -o /usr/local/bin/trojan
    fi
    ...
    if [[ ! -e /etc/systemd/system/trojan-web.service ]];then
        show_tip=1
        colorEcho $blue "正在下载服务文件..."
        if ! curl -L --connect-timeout 30 $service_url -o /etc/systemd/system/trojan-web.service; then
            colorEcho $yellow "使用备用链接下载服务文件..."
            curl -L --connect-timeout 30 "https://raw.githubusercontent.com/Jrohy/trojan/master/asset/trojan-web.service" -o /etc/systemd/system/trojan-web.service
        fi
        systemctl daemon-reload
